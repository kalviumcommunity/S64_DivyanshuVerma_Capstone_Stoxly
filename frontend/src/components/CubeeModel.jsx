...
function Model() {
  ...
  useFrame(() => {
    ...
    const { isRotating, pauseStartTime, currentFaceIndex, targetRotation } =
      rotationState;
    ...
    if (isRotating) {
      ...
      if (distanceToTarget < 0.05) {
        ...
        const direction =
          ...
          Math.PI;
        const step =
          Math.sign(direction) * Math.min(Math.abs(direction), rotationSpeed);
    ...
    >
      ...
      >
        <Suspense fallback={<Loader />}>
          ...
          {/* Single environment with optimized settings */}
